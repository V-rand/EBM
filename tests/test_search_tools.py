...
@pytest.mark.asyncio
async def test_web_search_returns_domain_drilldown_hint(monkeypatch):
    from agent_os.tools import search
    ...
    async def fake_tavily(*, query, max_results, timeout, extra):
        return {
            ...
            "detail": "ok",
            "results": [
                {"title": "A", "url": "https://www.example.org/path/a", "content": "x"},
                {"title": "B", "url": "https://archive.example.net/b", "content": "y"},
            ],
        }

    monkeypatch.setattr(search, "_try_tavily", fake_tavily)

    result = await search.handle_web_search(query='"rare phrase" method')

    assert result.success
    assert result.data["next_search_hint"]["mode"] == "domain_drilldown"
    assert result.data["next_search_hint"]["candidate_domains"][0] == "example.org"
    assert 'include_domains="example.org"' in result.data["next_search_hint"]["template"]


@pytest.mark.asyncio
async def test_web_search_omits_domain_drilldown_hint_when_already_domain_locked(monkeypatch):
    from agent_os.tools import search
    ...
    async def fake_tavily(*, query, max_results, timeout, extra):
        return {
            "status": "success",
            "detail": "ok",
            "results": [{"title": "A", "url": "https://example.org/path/a", "content": "x"}],
        }

    monkeypatch.setattr(search, "_try_tavily", fake_tavily)

    result = await search.handle_web_search(query='"rare phrase"', include_domains="example.org")

    assert result.success
    assert "next_search_hint" not in result.data
