...
@pytest.mark.asyncio
async def test_openalex_works_query_alias_uses_search_param(monkeypatch):
    from agent_os.tools import openalex
    ...
    monkeypatch.setattr(openalex, "_resolve_and_get_id", lambda *a, **k: fake_add())
    monkeypatch.setattr(openalex, "_resolve_work_ids", fake_resolve_ids)
    monkeypatch.setattr(openalex, "_openalex_get", fake_get)

    out = await openalex.handle_openalex_works(query="42,137 households ordinal probit")

    assert out.success
    assert captured["params"]["search"] == "42,137 households ordinal probit"
