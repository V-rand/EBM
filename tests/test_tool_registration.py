...
def test_model_request_disables_provider_parallel_tool_planning(tmp_path, monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "")
    monkeypatch.setenv("EMBEDDING_API_KEY", "")
    monkeypatch.setenv("DASHSCOPE_API_KEY", "")
    ...
    osys = AgentOS(data_dir=str(tmp_path))
    try:
        kwargs = osys.agent_loop._build_model_request_kwargs(
...
def test_tool_argument_parser_repairs_extra_closing_bracket():
    from agent_os.kernel.agent_loop import _parse_tool_arguments
    ...
    parsed, error = _parse_tool_arguments(raw)
