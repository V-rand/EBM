---
name: cat-example
description: "写 CAT 临床快评之前必加载。真实 Cochrane Plain Language Summary 范例——全文嵌入，不需要 web_read。展示如何用通俗语言写临床证据总结。"
---

# CAT 写作范例——Cochrane Plain Language Summary（完整真实范例）
...
1. **开头就是 Key messages**——不是 PICO 表，不是检索策略
2. **通俗语言解释专业概念**——"the large blood vessels of the legs are narrowed or blocked" 而不是 "atherosclerotic peripheral arterial disease"
3. **小标题是问题，段落是答案**——读者可以跳着读
4. **证据确定性单独一节**——不是藏在段落里，是读者关心的核心信息
5. **没有引用编号**——不打断阅读流
