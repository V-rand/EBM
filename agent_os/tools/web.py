...
async def handle_web_read(url, **kw) -> ToolResult:
    ...
    if mc and _should_use_mineru(url):
        if _is_pdf(url):
            result = await _race_pdf_parsers(mc, url)
            if result.success:
                return result
        else:
            ...
            for fut in done:
                ...
                if result["status"] == "success":
                    for f in tasks.values():
                        f.cancel()
                    return result
                errors.append(f"{name}: {result.get('status', 'error')} - {result.get('detail', 'no detail')}")
                tasks.pop(name, None)
        return {"status": "error", "detail": "; ".join(errors) or "all providers failed"}
    result = await _try_all()
    if result["status"] == "success":
        return ToolResult.ok(data={"url": url, "content": str(result["content"]).strip(), "parser": str(result.get("parser", ""))})

    msg = f"No reader provider returned usable content: {result.get('detail', 'all providers failed')}"
    return ToolResult.fail(msg)


async def _race_pdf_parsers(mc: MineruClient, url: str) -> ToolResult:
    tasks = {
        "mineru": asyncio.create_task(_exec_mineru(mc, url)),
        "pymupdf4llm": asyncio.create_task(_pdf_fallback(url)),
    }
    errors: list[str] = []
    while tasks:
        done, _ = await asyncio.wait(tasks.values(), return_when=FIRST_COMPLETED)
        for fut in done:
            name = next((n for n, task in tasks.items() if task is fut), "unknown")
            tasks.pop(name, None)
            try:
                result = fut.result()
            except Exception as exc:
            ...
            if isinstance(result, ToolResult):
                errors.append(f"{name}: {result.error or 'failed'}")
            elif result is None:
                errors.append(f"{name}: no usable content")
            else:
                errors.append(f"{name}: unexpected result")
    return ToolResult.fail("; ".join(errors) or "PDF parsers failed")
