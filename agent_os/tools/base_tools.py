        ...
        if normalized.startswith("research/") or normalized == "research":
            if not kw.get("confirm"):
                return ToolResult.fail(
                    "删除 research/ 下的文件需要 confirm=True 确认。"
                    "成果文件不应轻易删除。如确需清理，请显式传 confirm=True。"
                )
...
async def spawn(task, tools=None, subagent_type="general", **kw) -> ToolResult:
    ...
    if max_iterations is not None:
        try:
            ...
            return ToolResult.fail("max_iterations must be an integer")
    system_prompt = kw.get("system_prompt")
    try:
        sub_id, result = await agent_loop.spawn_sub_agent(task, tools, parent_session_id=parent_session_id, max_iterations=max_iterations, system_prompt=system_prompt)
        if not result.strip():
...
def register_base_tools(r) -> None:
    ...
    r.register("spawn", "execution", {
        ...
        "parameters": {"type": "object", "properties": {
            "task": {"type": "string", "description": "子任务描述（需详细说明背景、目标、期望输出），尽可能具体。"},
            ...
            "max_iterations": {"type": "integer", "description": "子Agent 最大推理轮次（默认 32）。引用校验等广度优先任务建议设为 15-20。"},
            "system_prompt": {"type": "string", "description": "覆盖子Agent的默认系统提示词。如预设的 citation_checker 提示词用于引用真实性校验。"},
        }, "required": ["task"]},
