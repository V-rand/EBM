...
- site: operator in query string for hard domain locking
...
async def handle_web_search(
    ...
    **kw,
    ...
    if result["status"] == "success":
        return ToolResult.ok(data=_format_search_data(query, result["results"], "tavily"))
    ...
    if result["status"] == "success":
        return ToolResult.ok(data=_format_search_data(query, result["results"], "serper"))
    if result["status"] == "error":
...
def _format_search_data(query: str, results: list[dict[str, Any]], provider: str) -> dict[str, Any]:
    data = {"query": query, "results": results, "count": len(results), "provider": provider}
    return data
...
def register_search_tools(r) -> None:
    ...
    "parameters": {"type": "object", "properties": {
        "query": {"type": "string", "description": "搜索查询，支持 site:domain, \"exact phrase\", -exclude, OR 操作符。用 site:xxx 硬性锁域。"},
