...
class MineruClient:
    def __init__(
        ...
        *,
        ...
        self._session.trust_env = False  # don't inherit global HTTP_PROXY
        if proxy:
    ...
    def _upload_file(self, file_url: str, source_path: Path) -> None:
        with source_path.open("rb") as handle:
            response = self._session.put(file_url, data=handle, timeout=self.timeout_seconds)
        response.raise_for_status()
    ...
    def _download_text(self, markdown_url: str) -> str:
        response = self._session.get(markdown_url, timeout=self.timeout_seconds)
        response.raise_for_status()
        return response.content.decode("utf-8", errors="replace")

    def _download_markdown_from_zip(self, full_zip_url: str) -> str:
        response = self._session.get(full_zip_url, timeout=max(self.timeout_seconds, 60))
        response.raise_for_status()
