...
from .agent_loop import _parse_tool_arguments
...
class SubAgent:
    ...
    async def run(self, task_description: str) -> str:
        ...
        from ..prompts import sub_agent as default_sub_agent
        ...
        messages = [{"role": "system", "content": (self._system_prompt or default_sub_agent) + runtime_rules}, {"role": "user", "content": task_description}]
        ...
        try:
            for iteration in range(self.max_iterations):
                ...
                if self.max_tool_calls is not None and self._tool_count >= self.max_tool_calls:
                    current_tools = []
                if current_tools:
                    request_kwargs["tools"] = current_tools
                    request_kwargs["tool_choice"] = "auto"
                    # Keep planning single-step to avoid accidental batches of
                    # quota-consuming retrieval calls such as web_search.
