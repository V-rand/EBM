...
def _parse_tool_arguments(arguments: Any) -> tuple[dict[str, Any] | None, str | None]:
    ...
    if not isinstance(arguments, str):
        return None, f"Invalid tool arguments type: {type(arguments).__name__}"
    try:
        parsed = json.loads(arguments)
    except json.JSONDecodeError:
        repaired = _repair_json_arguments(arguments)
        if repaired != arguments:
            try:
                parsed = json.loads(repaired)
            except json.JSONDecodeError:
                return None, f"Invalid JSON arguments: {arguments}"
        else:
            return None, f"Invalid JSON arguments: {arguments}"
    if not isinstance(parsed, dict):
        return None, f"Invalid tool arguments JSON: expected object, got {type(parsed).__name__}"
    return parsed, None


def _repair_json_arguments(arguments: str) -> str:
    """Best-effort repair for streamed tool JSON with stray closing delimiters.

    LLM tool calls often fail by emitting one extra ']' or '}' inside an otherwise
    valid object. We only skip unmatched closing delimiters outside strings; we do
    not invent missing fields or rewrite content.
    """
    stack: list[str] = []
    out: list[str] = []
    in_string = False
...
class AgentLoop:
    ...
    async def process(
        ...
        async with lock:
            ...
            # 清理上一轮的僵尸 subagent 状态目录
            subagents_dir = Path(session.work_dir) / "raw_search" / "subagents"
            ...
            try:
                ...
                while iteration < effective_max_iterations:
                    ...
                    _blocked, _reminder, consecutive_blind_search_rounds, guardrail_hint = self._research_search_guardrail(
                        tool_calls,
                    ...
                    if parallel_safe:
                        ...
                        for tc in tool_calls:
                            ...
                            if not ok:
                                ...
                                break
                            results = [raw]
                            async for evt in self._emit_tool_results_parallel([tc], results, session_id, iteration, message, messages, parallel=False):
                                yield evt
                    if not ok:
                        await self._rollback_orphaned_tool_call(session_id)
                        break
        ...
        *,
        ...
        if "research_state" in names and has_retrieval:
            messages.append({"role": "user", "content": (
                "<system-reminder>\n"
                "不要将 research_state 和检索工具放在同一轮调用。先单独调用 research_state。\n"
                "</system-reminder>"
            )})
            return False, "", 0, ""
        ...
        rounds_after_this_one = consecutive_blind_search_rounds + 1

        threshold = self.settings.blind_search_guardrail_rounds
        if rounds_after_this_one == threshold:
            return (
                False, "", rounds_after_this_one,
                f"已连续检索 {threshold} 轮。建议用 research_state 写下当前进展再继续。",
            )
        if consecutive_blind_search_rounds >= threshold and rounds_after_this_one % 2 == 0:
            messages.append({"role": "user", "content": (
                "<system-reminder>\n"
                f"已连续检索 {rounds_after_this_one} 轮未写草稿。建议用 research_state 审视已有发现。\n"
                "</system-reminder>"
            )})
        return False, "", rounds_after_this_one, ""
    ...
    def _build_model_request_kwargs(
        ...
        if chat_tools:
            ...
            # Keep planning single-step. This prevents the model from issuing
            # multiple web_search calls in one turn and burning search quota
            # before runtime per-tool concurrency checks can help.
    ...
    async def _execute_tool(self, tool_call: dict[str, Any], session_id: str, *,
        ...
        arguments, parse_error = _parse_tool_arguments(arguments)
        if parse_error:
            return ToolResult.fail(parse_error)
    ...
    async def _emit_tool_results_parallel(
        ...
        iteration: int, message: str, messages: list[dict], *, parallel: bool = True,
    ) -> AsyncGenerator[dict[str, Any], None]:
        for tc, raw_result in zip(tool_calls, results):
            ...
            detail_prefix = "并发耗时" if parallel else "耗时"
            yield {
                "type": "activity",
                "phase": "tool.completed",
                "detail": f"工具 {tc['name']} 执行完成 | {detail_prefix}: {(latency_ms or 0) / 1000:.1f}s",
                "payload": {"tool": tc["name"], "latency_ms": latency_ms, "success": result.success, "parallel": parallel},
            }
    ...
    async def _inject_turn_attachments(
        ...
        # -- 4. Todo nudge (after 8+ rounds without todowrite, every 4 rounds) ---
        if consecutive_tool_rounds > 7 and consecutive_tool_rounds % 4 == 0:
            refreshed = await self.sessions.get(session_id)
    ...
    async def spawn_sub_agent(
        ...
        *,
        ...
        sub_agent = SubAgent(
            api_key=self.api_key, base_url=self.base_url, model=model or self.model,
            ...
            sub_agent_work_dir=work_dir, reasoning_effort=reasoning_effort if reasoning_effort is not None else self._reasoning_effort,
            pending_messages=self._pending_messages,
            filter_tools=self._filter_tools,
            max_iterations=max_iterations or self.settings.sub_agent_max_iterations,
