    ...
    for msg in db_messages:
        ...
        if kind == "tool":
            ...
            if need_reasoning_roundtrip:
                ...
                if tool_calls is not None:
                    # DeepSeek thinking mode requires reasoning_content on ALL
                    # tool-call assistant messages. Some responses may not have
                    # produced thinking (e.g. repeated tool calls): use empty string.
                    item["reasoning_content"] = reasoning_content or ""
            result.append(item)
    ...
    while i < len(result):
        ...
        if expected != found:
            ...
            if valid:
                ...
                # DeepSeek requires reasoning_content even when tool_calls are pruned
                # Do NOT pop reasoning_content here
        i += 1
